...
logger = logging.getLogger(__name__) # Will be 'keylock.core'
...
def _get_font(preferred_fonts, base_size):
    ...
    if fp:
        try: return ImageFont.truetype(fp, base_size)
        except IOError: logger.warning(f"Font '{fp}' load failed. Defaulting.")
...
def generate_keylock_carrier_image(w=800,h=600,msg="KeyLock Wallet")->Image.Image:
    ...
    try:draw.text((tx+so,ty+so),msg,font=fnt,fill=tuple(sc),anchor="mm");draw.text((tx,ty),msg,font=fnt,fill=tc,anchor="mm")
    except AttributeError: # Fallback for older Pillow versions (no anchor="mm")
        ...
        draw.text((ax+so,ay+th),msg,font=fnt,fill=tuple(sc));draw.text((ax,ay),msg,font=fnt,fill=tc)
    return img

def _get_text_width(draw_obj, text_str, font_obj):
    if hasattr(draw_obj, 'textlength'): # Pillow 9.2.0+
        return draw_obj.textlength(text_str, font=font_obj)
    elif hasattr(draw_obj, 'textbbox'): # Pillow 8.0.0+ for (0,0) anchor
        bbox = draw_obj.textbbox((0,0), text_str, font=font_obj)
        return bbox[2] - bbox[0]
    else: # Older Pillow
        return draw_obj.textsize(text_str, font=font_obj)[0]
...
def draw_key_list_dropdown_overlay(image: Image.Image, keys: list[str] = None, title: str = "Data Embedded") -> Image.Image:
    ...
    box_w_max_target = int(image.width * 0.30)
    title_font_size = max(10,min(int(image.height*0.028),int(box_w_max_target*0.12))); title_font=_get_font(PREFERRED_FONTS,title_font_size)
    key_font_size = max(9,min(int(image.height*0.025),int(box_w_max_target*0.1))); key_font=_get_font(PREFERRED_FONTS,key_font_size)
    
    title_w_actual = _get_text_width(draw, title, title_font)
    # Use textbbox to get height, textsize for older Pillow if textbbox not available
    ...
    title_h_actual = title_bbox_for_h[3]-title_bbox_for_h[1] if hasattr(draw,'textbbox') else draw.textsize(title,font=title_font)[1]
    ...
    if keys:
        ...
        for kt in temp_disp_keys:
            ...
            kh = k_bbox_for_h[3]-k_bbox_for_h[1] if hasattr(draw,'textbbox') else draw.textsize(kt,font=key_font)[1]
    ...
    # Position TOP-LEFT (original was top-right, changing based on description in demo.py)
    title_bar_x0 = margin
    title_bar_x1 = title_bar_x0 + content_box_w
    title_bar_y0 = margin
    title_bar_y1 = title_bar_y0 + title_bar_h
    
    # Ensure it doesn't exceed image boundaries (especially for small images)
    title_bar_x1 = min(title_bar_x1, image.width - margin)
    title_bar_y1 = min(title_bar_y1, image.height - margin)
    content_box_w = title_bar_x1 - title_bar_x0 # Recalculate actual width

    draw.rectangle([(title_bar_x0,title_bar_y0),(title_bar_x1,title_bar_y1)],fill=title_bg_color)
    # Center title text in the possibly resized title bar
    title_text_x = title_bar_x0 + (content_box_w - min(title_w_actual, content_box_w - 2*padding['title_x'])) / 2
    ...
    if disp_keys:
        ...
        key_list_x0, key_list_x1 = title_bar_x0, title_bar_x1 # Match title bar width
        key_list_y0 = title_bar_y1 # Position below title bar
        ...
        current_key_list_box_h = key_list_y1 - key_list_y0 # Actual height
        current_content_box_w = key_list_x1 - key_list_x0 # Actual width (same as title bar)

        draw.rectangle([(key_list_x0,key_list_y0),(key_list_x1,key_list_y1)],fill=key_list_bg_color)
        
        cty = key_list_y0 + padding['key_y']
        available_text_width_for_keys = current_content_box_w - 2 * padding['key_x']

        for i, key_text_item in enumerate(disp_keys):
            # Ensure key_line_heights[i] exists
            if i >= len(key_line_heights): break 
            
            if cty + key_line_heights[i] > key_list_y0 + current_key_list_box_h - padding['key_y']: # Vertical fit check
                ellipsis_h_bbox = draw.textbbox((0,0),"...",font=key_font) if hasattr(draw,'textbbox') else (0,0,0,0)+draw.textsize("...",font=key_font)
                ellipsis_h = ellipsis_h_bbox[3]-ellipsis_h_bbox[1] if hasattr(draw,'textbbox') else draw.textsize("...",font=key_font)[1]
                
                if cty + ellipsis_h <= key_list_y0 + current_key_list_box_h - padding['key_y']:
                    ellipsis_w = _get_text_width(draw,"...",key_font)
                    draw.text((key_list_x0 + (current_content_box_w - ellipsis_w)/2, cty), "...", font=key_font, fill=ellipsis_color)
                break # Stop drawing more keys if space runs out
            
            current_key_text_w_original = actual_key_text_widths[i]
            text_to_draw = key_text_item
            
            if current_key_text_w_original > available_text_width_for_keys:
            ...
            current_key_text_w_final = _get_text_width(draw, text_to_draw, key_font)
            # Center text within the available key text area
            text_x_pos = key_list_x0 + padding['key_x'] + max(0, (available_text_width_for_keys - current_key_text_w_final) / 2)
            ...
            draw.text((text_x_pos, cty), text_to_draw, font=key_font, fill=text_color_to_use)
            cty += key_line_heights[i]
            if i < len(disp_keys)-1: cty += line_spacing
