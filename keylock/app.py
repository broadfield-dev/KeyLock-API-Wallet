...
# Ensure gr.themes.colors is accessible, provide fallback if not (e.g. very old Gradio)
try:
    ...
    # Define some basic fallback colors if gr.themes.colors is not available
    class FallbackColors:
        ...
        cyan = "cyan"
        neutral = "gray"
    blue_color = sky_color = slate_color = cyan_color = neutral_color = FallbackColors()
...
try:
    ...
    light_theme = None # Gradio will use its default

try:
    dark_theme = gr.themes.Base( # Base is for Gradio 3.x
        primary_hue=blue_color,
    ...
    dark_theme = None # No specific dark theme if Base is unavailable
...
def gradio_embed_data(kv_string: str, password: str, 
    ...
    try:
        ...
        carrier_img = (core.generate_keylock_carrier_image() if generate_carrier_flag or input_image_pil is None
                       else input_image_pil.copy())
        ...
        final_carrier = core.draw_key_list_dropdown_overlay(
            carrier_img, 
        ...
        stego_final_img = core.embed_data_in_image(final_carrier, encrypted_data)
        ...
        temp_fp = None
        # Use tempfile context manager correctly
        ...
        # Return the path for gr.File to handle
...
def gradio_extract_data(stego_image_pil: Image.Image, password: str):
    ...
    try:
        if stego_image_pil.format != "PNG": app_logger.warning(f"Uploaded image format '{stego_image_pil.format}' not PNG. LSB data may be compromised if not original file.")
        extracted_data = core.extract_data_from_image(stego_image_pil)
    ...
    with gr.Blocks(theme=light_theme, title="KeyLock Secure Steganography") as keylock_app_interface:
        ...
        with gr.Tabs():
            with gr.TabItem(f"{ICON_EMBED} Embed Data"):
                with gr.Row(equal_height=False):
                    with gr.Column(scale=2, min_width=350):
                        ...
                        gr.HTML("<h3 style='margin-bottom:8px;margin-top:15px;'>Carrier Image</h3>")
                        ...
                        gr.HTML("<h3 style='margin-bottom:8px;margin-top:15px;'>Visual Markings (Top-Right)</h3>")
                        ...
                        gr.HTML("<p style='font-size:0.9em;margin-bottom:5px;'>A 'KeyLock: Data Embedded' title bar will always be present above the optional key list.</p>")
                        gr.HTML("<h3 style='margin-bottom:8px;margin-top:15px;'>Output</h3>")
                    ...
                    with gr.Column(scale=3, min_width=450):
                        ...
                        download_stego_file = gr.File(label="Download Your KeyLock Image (PNG)",interactive=False) # No type needed, will be path
            ...
            with gr.TabItem(f"{ICON_EXTRACT} Extract Data"):
                with gr.Row(equal_height=False):
                    with gr.Column(scale=2, min_width=350):
                        input_stego_extract = gr.Image(label="Upload KeyLock Stego Image (Unmodified PNG from download)",type="pil",sources=["upload","clipboard"])
        ...
        gr.Markdown("""---<div style="max-width:750px;margin:15px auto;font-size:0.95em;"><h3 style='margin-bottom:10px;text-align:left;'>Important Notes:</h3><ul style="padding-left:20px;text-align:left;line-height:1.6;"><li><strong>Use Downloaded PNG for Extraction:</strong> Copy-pasting images from browser can corrupt LSB data.</li><li><strong>PNG Format is Crucial:</strong> Non-PNG or re-compressed images will likely lose data.</li><li><strong>Password Security:</strong> Use strong, unique passwords.</li><li><strong>Data Capacity:</strong> Visuals reduce capacity.</li></ul></div>""")
...
def main():
    ...
    try:
        ...
        app_logger.info("System font (Arial/DejaVuSans) available for PIL.")
    except IOError:
